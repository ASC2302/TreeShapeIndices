...
calculate_all_subtree_indices <- function(
  ...
  for (tree_i in seq_along(tree_list)) {
    ...
    tree <- normalize_phylo(tree_list[[tree_i]])
    ...
    sts <- lapply(sts, normalize_phylo)
    ...
    for (sub_i in seq_along(sts)) {
      ...
      timing <- system.time({
        idx <- tryCatch(
          all_indices(st),
          error = function(e) {
            warning(paste("Error for tree", tree_i, "subtree", sub_i, ":", e$message))
            return(NULL)
          }
        )
      })
